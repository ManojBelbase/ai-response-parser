...
function parseMarkdown(text) {
    ...
    html = html.replace(tableRegex, (match, headerRow, bodyRows) => {
        if (!headerRow.includes('|'))
            return match;
        const headers = headerRow.split('|').slice(1, -1).map((h) => `<th>${h.trim().replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')}</th>`).join('');
        const rows = [];
        if (bodyRows) {
            bodyRows.split('\n').forEach((row) => {
                if (row.includes('|')) {
                    const cells = row.split('|').slice(1, -1).map((cell) => `<td>${cell.trim().replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')}</td>`).join('');
